...
namespace Utils
{
    public class ScheduleExcelUtils : IScheduleUtils
    {
        ...
        //public List<ScheduleDetailsDTO> Read(Stream excelStream)
        //{
        ...
        public byte[] Write(List<List<Dictionary<string, int>>> objList)
        {
            if (objList != null)
            {
        ...
        //public byte[] Write(List<ScheduleDetailsDTO> schedulesList)
        //{
        ...
        private List<Cell> GenerateCellsFromItems(List<List<Dictionary<string,int>>> objList, SheetData sheetData)
        {
            ...
            foreach (var obj in objList) //single object
            {
                ...
                foreach (var dict in obj)
                {
                    ...
                    {
                        var property = field.Key;
                        var type = field.Value;
                        string cellAddress = columnID + rowID.ToString();
                        Cell cell = SetCell(property, type, cellAddress);
                        row.Append(cell);
                        cellList.Add(cell);
        ...
        //todo: enum casting from cellvalues to datatype
        private Cell SetCell(string cellValue, int cellDataType, string cellAddress)
        {
            Cell cell = new Cell()
            {
                CellValue = new CellValue(cellValue),
                DataType = new EnumValue<CellValues>((CellValues)cellDataType),
                CellReference = cellAddress,
                StyleIndex = GetCellStyleIndex((CellValues)cellDataType)
            };
            return cell;
        }
